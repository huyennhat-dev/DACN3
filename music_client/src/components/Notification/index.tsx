...
const NotiItem = ({
  ...
  return (
    <div
      onClick={confirmNoti}
      className={`relative my-1 rounded hover:bg-primary-50/20 p-2 cursor-pointer ${
        !data.active && "bg-grey-500/10"
      }`}
    >
      <p className="text-xs line-clamp-1 overflow-hidden font-medium mb-1">
        {data.content?.title}
      </p>
      <p className="text-xs">{data.content?.message}</p>
      <p className="text-end text-xs">
        {data.createdAt ? formatRelativeTime(data.createdAt!) : "Vừa xong"}
      </p>
      {!data.active && (
        <div className="absolute top-1 right-1 w-3 h-3 rounded-full bg-primary-100"></div>
      )}
    </div>
  );
};
  ...
  useEffect(() => {
    ...
    if (info) {
      ...
      socket.on("new-notify", (newNotify) => {
        if (newNotify.user == info.id) {
          setNotiData((prev) => [newNotify, ...prev]);
        }
      });
          ...
          {notiData.length > 0 ? (
            notiData.map((noti) => (
              <NotiItem
                key={noti._id}
                data={noti}
                confirmNoti={() => handleConfirmNoti(noti._id!)}
              />
            ))
          ) : (
